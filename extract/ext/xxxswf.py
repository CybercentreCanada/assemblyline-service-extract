"""Reads and extracts flash archives.

Taken from https://github.com/viper-framework/xxxswf/blob/master/xxxswf/xxxswf.py
----------------------------------------
Name:
    xxxswf.py
Version:
...
class xxxswf:
    ...
    def uncompress_lzma(self, data):
        """Uncompress lzma compressed stream."""  # noqa:DOC201
    ...
    def uncompress_zlib(data):
        """Uncompress zlib compressed stream."""  # noqa:DOC201
        try:
    ...
    def verify_swf(self, stream, addr):
        """Carve and verify embedded SWF."""  # noqa:DOC201
        ...
        if header == b"FWS":
            ...
            if uncompress_data is None:
                if self.debug:
                    self.log.debug(
                        "\t\t[DEBUG] Zlib decompession failed",
                    )
                return None
            ...
            if uncompress_lzma is None:
                if self.debug:
                    self.log.debug(
                        "\t\t[DEBUG] lzma decompession failed",
                    )
                return None
            stream.seek(addr + 3)
    ...
    def pre_file_scan(self, data, file_scan):
        """Executes a user defined function."""  # noqa:DOC201
    ...
    def compress_lzma(self, swf):
        """Compress a SWF with LZMA."""  # noqa:DOC201
        if type(swf) is bytes:
            swf = BytesIO(swf)
        if not self.lzma_install:
            if self.show_errors:
    ...
    def get_arguments(self):
        ...
        parser.add_option(
            ...
            dest="extract",
            help=(
                "Extracts the embedded SWF(s), names it MD5HASH.swf & saves "
        ...
        parser.add_option(
            ...
            dest="md5scan",
            help=(
                "Scans the SWF(s) for MD5 signatures. Please see func check "
        ...
        parser.add_option(
            "-d", "--decompress", action="store_true", dest="decompress", help="Deflates compressed SWFS(s)"
        )
        parser.add_option(
            "-r",
            "--recdir",
    ...
    def create_unique_name(self, name, ext):
        """Create a unique filename if filename already exists."""  # noqa:DOC201
        count = None
    ...
    def md5_hash_buffer(self, data):
        """MD5 hashes a buffer."""  # noqa:DOC201
    ...
    def process(self, stream, file_name=None):
        ...
        for index, swf_addr in enumerate(swf_data):
            if self.cmd_run:
                self.log.info(
                    "\t[ADDR] SWF %d at %s" % (index + 1, hex(swf_addr)),
                )
            # set read to the address of the SWF header found
